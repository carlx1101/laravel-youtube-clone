<template>
    <div class = "relative">
        ...
        <div @mouseover = "show = true" @mouseleave="show = false; showVideo = false">
            <img :src="thumbnail || ''" class="aspect-video cursor-pointer" :class="show ? 'transition ease-in-out delay-150 rounded-l-lg' : 'rounded-lg', showVideo ? 'delay-350 hidden' : ''">
            <div class="w-full h-full aspect-video cursor-pointer" :class=" showVideo ?  '': 'delay-350 hidden'"  >
                <video :src="videoUrl || ''" type="video/mp4" ref="video" />
            </div>
        </div>  
...
//  Play video when hovered, restart to the start point of the video when doesnt hover
